...
function getOpenRouterConfig() {
  ...
  return {
    ...
    title: process.env.OPENROUTER_APP_NAME || "AnonKonnect",
  };
}
...
async function requestOpenRouter({ personaId, message, history = [], userGender }) {
  ...
  if (!response.ok) {
    const errorMessage =
      payload?.error?.message || payload?.error || `OpenRouter request failed with status ${response.status}.`;
    throw new Error(errorMessage);
  }
...
export async function generatePersonaReply(input) {
  if (!process.env.GEMINI_API_KEY && !getSecondaryProviderConfig() && !getOpenRouterConfig()) {
    return {
  ...
  try {
    ...
    if (!shouldFallbackToSecondary(error)) {
      throw error;
    }

    if (getSecondaryProviderConfig()) {
    ...
    if (getOpenRouterConfig()) {
      const openRouterResult = await requestOpenRouter(input);
      return {
        ...openRouterResult,
        fallbackFrom: "gemini",
      };
