...
function getOppositeGender(userGender) {
  ...
  return null; // neutral for non-binary / prefer-not-to-say / empty
}

function buildGenderSystemInstruction(userGender) {
  const opposite = getOppositeGender(userGender);
  if (!opposite) {
    return "";
  }

  return `Adopt an opposite-gender voice (${opposite}). Use pronouns and a tone that fits ${opposite}. Keep it respectful, casual, and short.`;
}
...
async function requestGemini({ personaId, message, history = [], userGender = "" }) {
  ...
  const genderInstruction = buildGenderSystemInstruction(userGender);
  const systemInstruction = genderInstruction
