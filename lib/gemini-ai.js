...
async function requestGemini({ personaId, message, history = [] }) {
...
async function requestSecondaryProvider({ personaId, message, history = [] }) {
  ...
  const response = await fetch(`${providerConfig.baseURL}/chat/completions`, {
    ...
    body: JSON.stringify({
      ...
      messages: mapHistoryToOpenAIMessages(persona, history, message),
      temperature: personaId === "jokebot" || personaId === "roleplay" ? 0.9 : 0.7,
      max_tokens: 500,
    }),
    cache: "no-store",
